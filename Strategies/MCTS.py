...
#Implementation of the Monte Carlo Tree Search (MCTS) according to https://www.geeksforgeeks.org/machine-learning/ml-monte-carlo-tree-search-mcts/
...
class MCTSNode:
    def __init__(self, board, last_move, player_turn, parent=None, move=None, scores=(0, 0)):
        ...
        self.untried_actions = self.get_actions()    #moves to be explored in the algo
        random.shuffle(self.untried_actions)         #shuffle to ensure random exploration
    ...
    def get_available_moves(board, last_move):
        ...
        for col, val in enumerate(board[last_r]):
            if val != 0:
    ...
    def get_actions(self):
        """
        use static method above on its own data.
        """
        return MCTSNode.get_available_moves(self.board, self.last_move)
    
    def is_terminal(self):
        """Check if the game has ended."""
        #ends if list of untried actions from node on is empty and have not created any further nodes from here on
        return len(MCTSNode.get_available_moves(self.board, self.last_move)) == 0
    ...
    def expand(self):
        ...
        action = self.untried_actions.pop()                 #take last move from moves list and remove it
        copied_board = deepcopy(self.board)                 #want to simulate future without messing up present-> copy
    ...
    def best_child(self):
        ...
        for child in self.children:
            ...
            exploration_term = UCB_CONST * math.sqrt(math.log(max(self.visits, 1)) / child.visits)
...
class MCTSStrategy(Strategy):
    ...
    def __init__(self, max_iterations=10000, time_limit=5.0):
        ...
        Efficienc< limit ->CPU doesn't keep working if it has already thought enough
        """
    ...
    def move(self, board, last_move, scores):
        ...
        while (time.time() - start_time < self.time_limit) and (iteration_count < self.max_iterations):
            ...
            #Expansion: at leavev pick one move, it hasn't tried before and create a new child node for it
            if not leaf_node.is_terminal():
                #expanded node should become the new leaf for the simulation.
                expanded_node = leaf_node.expand()
                if expanded_node:
                    leaf_node = expanded_node

            #Simulation: AI plays randomly until the game ends
            result = self.simulate(leaf_node, total_sum)
        ...
        #select node with highest mean value  
        best_child = max(root.children, key=lambda c: (c.score_total / c.visits))
        return best_child.move

    
    def select(self, node):
        """
        Selection phase as described above
        """
        current_node = node

        #only go further dowm in the tree if it's not an end node AND the node is fully expanded already
        while not current_node.is_terminal() and current_node.is_fully_expanded():
            current_node = current_node.best_child()
        
        #stopped loop because either found a node with untried moves (->expand) or hit a terminal node (->backpropagate)
        return current_node
    ...
    def simulate(self, node, total_sum):
        ...
        copied_board = deepcopy(node.board)                 #want to simulate future without messing up present-> copy
        copied_last_move = node.last_move                   #get copy of last move played
        copied_scores = list(node.scores)                     #get copy of nr of wins
        copied_player_turn = node.player_turn                #get copy of player, whose turns it is
        ...
        while True:
            ...
            copied_board[move[0]][move[1]] = 0              #erase spot availability from copied board
            copied_scores[copied_player_turn - 1] += move_value  #add value of move to the copied score of player
            #update move and player
            copied_last_move = move
            copied_player_turn = 3 - copied_player_turn

        #calculate final scores from the perspective_player's point of view
        player_final_score = copied_scores[perspective_player - 1]
        opponent_final_score = copied_scores[2 - perspective_player]

        #return score difference
        raw = player_final_score - opponent_final_score
